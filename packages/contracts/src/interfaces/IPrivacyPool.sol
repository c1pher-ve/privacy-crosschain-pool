...
interface IPrivacyPool is IState {
  ...
  /*///////////////////////////////////////////////////////////////
                      CROSS-CHAIN STATE METHODS
  //////////////////////////////////////////////////////////////*/

  /**
   * @notice Returns whether a root is known in the root history
   * @dev Used by CrossChainEntrypoint to verify proofs against known pool state
   * @param _root The root to check
   * @return _known Whether the root exists in the history
   */
  function isKnownRoot(uint256 _root) external view returns (bool _known);

  /**
   * @notice Marks a nullifier as spent without transferring funds
   * @dev Used by CrossChainEntrypoint to burn a commitment on the source chain.
   *      Only callable by the Entrypoint.
   * @param _nullifierHash The nullifier hash to spend
   */
  function burnNullifier(uint256 _nullifierHash) external;

  /**
   * @notice Inserts a cross-chain commitment into the state tree without pulling funds
   * @dev Used by CrossChainEntrypoint to mint a commitment on the destination chain.
   *      The pool must be pre-funded with liquidity. Only callable by the Entrypoint.
   * @param _commitment The commitment hash to insert
   */
  function insertCrossChainCommitment(uint256 _commitment) external;

  /**
   * @notice Releases excess locked assets to the Entrypoint custody contract
   * @dev Used by CrossChainEntrypoint to pull burned funds out of the pool during burn operations.
   *      Only callable by the Entrypoint.
   * @param _recipient The address receiving the assets (typically the Entrypoint)
   * @param _value The amount of assets to release
   */
  function releaseCrossChainAssets(address _recipient, uint256 _value) external;
}
