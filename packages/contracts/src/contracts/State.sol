...
abstract contract State is IState {
   ...
   * @dev Public wrapper around the internal _isKnownRoot for cross-chain verification
   ...
   */
  function burnNullifier(uint256 _nullifierHash) external onlyEntrypoint {
   ...
   */
  function insertCrossChainCommitment(uint256 _commitment) external onlyEntrypoint {
    _insert(_commitment);
  }
