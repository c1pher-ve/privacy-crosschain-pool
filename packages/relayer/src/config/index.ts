...
/**
 * Gets the cross-chain entrypoint address for a chain.
 *
 * @param {number} chainId - The chain ID
 * @returns {string | undefined} The cross-chain entrypoint address
 */
export function getCrossChainEntrypointAddress(chainId: number): string | undefined {
  const chainConfig = getChainConfig(chainId);
  return chainConfig.cross_chain_entrypoint_address;
}
